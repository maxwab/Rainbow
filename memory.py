...
def save_mem(mem, p):
  ...
  with h5py.File(p / 'mem_data.h5', 'w') as f:
    f.create_dataset("data", data=mem.transitions.data, compression="gzip")
    f.create_dataset("sum_tree", data=mem.transitions.sum_tree, compression="gzip")
...
def load_mem(mem, p):
  ...
  for k, v in transitions_dic.items():
    setattr(mem.transitions, k, v)

  # Remaining: the datasets
  with h5py.File(p / 'mem_data.h5', 'r') as f:
    setattr(mem.transitions, 'data', np.array(f['data']))
    setattr(mem.transitions, 'sum_tree', np.array(f['sum_tree']))
